/****************************************************************************
*
*	erbga.cpp:	Code for finding communities using Genetic Programming. 
...
GA::GA(Network &sparseNetwork, int popSize, int generations, int numNodes, int numEdges)
{
	...
	std :: cout << "Start from checkpoint population file? (y/n) : ";
	std :: cin >> choice;
	if ( choice == 'y' ) {
		std :: cout << "\n\tCheckpoint file name : ";
		std :: cin >> checkpoint_filename;
		std :: cout << "\nImporting from : " << checkpoint_filename;

		std::fstream checkpoint_file_ptr;
		checkpoint_file_ptr.open(checkpoint_filename , std::ios::in);
		if( checkpoint_file_ptr.is_open() ) {
			std::cout << "\n--- Reading file...\n";
	...
	// generate initial population
		for (int i = 0; i < populationSize; ++i)
		{
			...
			for (int j = 0; j < networkNumEdges; ++j)
			{
				...
				double degree_rate_u = gaSparseNetwork->vertices[originalEdgeIDS[j] / networkNumVertices].degree_rate;
				double degree_rate_v = gaSparseNetwork->vertices[originalEdgeIDS[j] % networkNumVertices].degree_rate;

				double degree_factor = 0;

				if ( degree_rate_u == 0.0f && degree_rate_v == 0.0f )
					degree_factor = 0;
				else if (degree_rate_u == 0.0f || degree_rate_v == 0.0f) {
					if ( degree_rate_u == 0 )
						degree_factor = degree_rate_v;
					else
						degree_factor = degree_rate_u;
				} else {
					degree_factor = (degree_rate_u + degree_rate_v) / 2;
				}


				if (r < round((GA_RANDOM_POP_PERCENT + (degree_factor) ) * 100))
				{
					set_bit(i, j, prev);
					hits++;
...
void GA::exportPopulationData(int next) {
	std::fstream file; // declare an object of fstream class
	char filename[256] = {0};
	strcpy(filename, GA_POPULATION_DATA.c_str());
	strcat(filename, dataset_name);
	strcat(filename, the_date);
	file.open(filename, std::ios::out); // open file in append mode

	for (int i = 0; i < populationSize; ++i)
	{
		for (int j = 0; j < networkNumEdges; ++j)
		{
			file << get_bit(i, j, next) << "\t";
		}
		file << std::endl;
	}
	file.close();
